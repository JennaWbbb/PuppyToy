    ...
    public class EmlalockFeedReader
    {
        ...
        private LockActionType GetLockActionTypeFromTitle(string? title)
        {
            ...
            else if (title.Contains("Friend Link") || title.Contains("voting from"))
            {
                ...
                // If durations are disabled, we don't know which way they voted, just that they voted.
                return LockActionType.Voted;
            }
            ...
            else if (title.Contains("for not playing the number of game"))
            {
                return LockActionType.InsufficientGames;
            }
            else if (title.Contains("for looking at the remaining duration"))
            {
                return LockActionType.RemainingDuration;
            }
            else if (title.Contains("duration changed from"))
            {
                if (title.Contains("(+"))
                {
                    return LockActionType.HolderAdded;
                }
                else if (title.Contains("(-"))
                {
                    return LockActionType.HolderRemoved;
                }

                // If durations are disabled, we don't know which way they voted, just that they voted.
                return LockActionType.HolderChanged;
            }
